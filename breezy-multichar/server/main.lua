...
QBCore.Commands.Add("logout", "Logout of Character (Admin Only)", {}, false, function(source)
    ...
    TriggerClientEvent('breezy-multichar:client:chooseChar', src)
end, "admin")

QBCore.Commands.Add("closeNUI", "Close Multi NUI", {}, false, function(source)
    local src = source
    TriggerClientEvent('breezy-multichar:client:closeNUI', src)
end)

-- Events

RegisterNetEvent('breezy-multichar:server:disconnect', function()
    local src = source
    DropPlayer(src, "You have disconnected from QBCore")
end)

RegisterNetEvent('breezy-multichar:server:loadUserData', function(cData)
    local src = source
...
RegisterNetEvent('breezy-multichar:server:createCharacter', function(data)
    local src = source
    local newData = {}
    ...
    if QBCore.Player.Login(src, false, newData) then
        if Config.StartingApartment then
            ...
            QBCore.Commands.Refresh(src)
            loadHouseData()
            TriggerClientEvent("breezy-multichar:client:closeNUIdefault", src)
            GiveStarterItems(src)
        end
	end
end)

RegisterNetEvent('breezy-multichar:server:deleteCharacter', function(citizenid)
    local src = source
...
QBCore.Functions.CreateCallback("breezy-multichar:server:GetUserCharacters", function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback("breezy-multichar:server:GetServerLogs", function(source, cb)
    MySQL.Async.fetchAll('SELECT * FROM server_logs', {}, function(result)
        cb(result)
    end)
end)

QBCore.Functions.CreateCallback("breezy-multichar:server:setupCharacters", function(source, cb)
...
QBCore.Functions.CreateCallback("breezy-multichar:server:getSkin", function(source, cb, cid)
